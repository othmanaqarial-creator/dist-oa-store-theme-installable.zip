...
<body class="bg-slate-50 text-slate-900">
    ...
    <script>
        const USD_RATE = 0.27;
        const DEFAULT_ORDER_EMAIL = <?php echo wp_json_encode($oa_order_email); ?>;
        const state = {
            "language": "ar",
            "checkout": null
        };
        ...
        function handleSendOrder() {
            ...
            const mailtoEmail = state.checkout.orderEmail || DEFAULT_ORDER_EMAIL;
            const mailto = `mailto:${mailtoEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(summaryLines.join("\n"))}`;
