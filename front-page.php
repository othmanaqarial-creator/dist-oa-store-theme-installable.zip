?php
$oa_order_email = oa_store_get_order_email();
$oa_checkout_url = oa_store_get_checkout_url();
$oa_wp_products = array();
$oa_wp_categories = array();
$oa_category_styles = array(
    0 => array('icon' => 'fas fa-box-open', 'color' => 'bg-slate-100 text-slate-600'),
    1 => array('icon' => 'fas fa-laptop-code', 'color' => 'bg-blue-100 text-blue-600'),
    2 => array('icon' => 'fas fa-mobile-alt', 'color' => 'bg-green-100 text-green-600'),
    3 => array('icon' => 'fas fa-tshirt', 'color' => 'bg-purple-100 text-purple-600'),
    4 => array('icon' => 'fas fa-couch', 'color' => 'bg-yellow-100 text-yellow-600'),
    5 => array('icon' => 'fas fa-heartbeat', 'color' => 'bg-pink-100 text-pink-600'),
    6 => array('icon' => 'fas fa-gamepad', 'color' => 'bg-indigo-100 text-indigo-600'),
    7 => array('icon' => 'fas fa-book-open', 'color' => 'bg-red-100 text-red-600'),
    8 => array('icon' => 'fas fa-dumbbell', 'color' => 'bg-teal-100 text-teal-600'),
);
...
if (class_exists('WooCommerce') && function_exists('wc_get_products')) {
    ...
    foreach ($products as $product) {
        ...
        $category_name = $primary_term ? $primary_term->name : __('Uncategorized', 'oa-store-theme');

        if (!isset($seen_categories[$category_slug])) {
            $style = $oa_category_styles[$category_index % count($oa_category_styles)];
            $oa_wp_categories[] = array(
                'id' => $category_slug,
            ...
            $seen_categories[$category_slug] = true;
            $category_index++;
        }
        ...
        if (empty($image_url)) {
            $image_url = 'https://picsum.photos/320/220?random=1299';
        }
        ...
        if ($sale_price > 0 && $regular_price > $sale_price) {
            $badge = __('عرض خاص', 'oa-store-theme');
        }
        ...
        $oa_wp_products[] = array(
            ...
            'nameAr' => $product->get_name(),
            'nameEn' => $product->get_name(),
    ...
    <script type="application/ld+json">
        {
            ...
            "email": "<?php echo esc_html($oa_order_email); ?>",
...
<body class="bg-slate-50 text-slate-900 transition-colors duration-300">
                    ...
                    <div class="space-y-4">
                        <div class="bg-white bg-opacity-15 rounded-2xl p-5 flex items-center space-x-4 space-x-reverse">
                            ...
                            <div>
                                ...
                                <p id="orderEmailDisplay" class="text-lg font-semibold"><?php echo esc_html($oa_order_email); ?></p>
                            </div>
                        </div>
                        <div class="bg-white bg-opacity-15 rounded-2xl p-5 flex items-center space-x-4 space-x-reverse">
                            <div class="w-12 h-12 rounded-full bg-white bg-opacity-20 flex items-center justify-center">
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-xs text-white text-opacity-70">
                        ...
                        <li class="flex items-center space-x-3 space-x-reverse">
                            ...
                            <span id="footerEmailDisplay"><?php echo esc_html($oa_order_email); ?></span>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <script>
        ...
        const state = {
            ...
            "orderEmail": localStorage.getItem("oa_order_email") || <?php echo wp_json_encode($oa_order_email); ?>,
        ...
        const WP_PRODUCTS = <?php echo wp_json_encode($oa_wp_products); ?>;
        const WP_CATEGORIES = <?php echo wp_json_encode($oa_wp_categories); ?>;

        if (Array.isArray(WP_PRODUCTS) && WP_PRODUCTS.length > 0) {
            state.products = WP_PRODUCTS;
        }

        if (Array.isArray(WP_CATEGORIES) && WP_CATEGORIES.length > 0) {
            state.categories = WP_CATEGORIES;
        }
        ...
        function handleCheckoutNavigation() {
            ...
            window.location.href = "<?php echo esc_url($oa_checkout_url); ?>";
        }
